...
MyCommuteTrain.prototype.go = function() {
    ...
    if (me.stationRequest.Origin && me.stationRequest.Destination) {
        ...
        me.metra.reqTrainData(me.stationRequest, function(data) {
        ...
        me.metra.reqTrainData(stationReq, function(data2) {
...
MyCommuteTrain.prototype.showTrainInfo = function() {
    ...
    function obj2html(stationObj) {
        if (!(stationObj && stationObj.sched && stationObj.est)) {
            me.debug && console.log('obj2html() skipping invalid train', stationObj);
            return '-';
        }
        var sTime = me.uiTime(stationObj.sched);
        var eTime = me.uiTime(stationObj.est);

        var html = eTime + ' (' + (sTime == eTime ? 'same' : sTime) + ')';
        html += '<br><span data-mins="' + me.fromNow(stationObj.est) + '">';
        html += me.fromNow(stationObj.est, 1) + '</span>';
        return html;
    }
    ...
    $.each(me.trainInfo.parsedData, function(train_num, trainObj) {
        if (!trainObj[me.stationRequest.Origin]) {
            ...
            console.log("showTrainInfo() TODO mins till next train in origin", 
                me.fromNow(trainObj[me.stationRequest.Origin].est));
        }
        html += '<tr>';
        html += '<td>' + train_num + '</td>';
        html += '<td class="origin">' + obj2html(trainObj[me.stationRequest.Origin]) + '</td>';
        html += '<td class="destination">' + obj2html(trainObj[me.stationRequest.Destination]) + '</td>';
        html += '</tr>';
    });
    ...
    $('.lastUpdated').html(me.uiTime(me.trainInfo.updated, true));
};
...
MyCommuteTrain.prototype.processTrainTimes = function(data) {
    ...
    me.trainInfo = me.metra.aggregateTrainInfo(data, me.trainInfo);
    if (!me.trainInfo.valid) {
        me.debug && console.log("processTrainTimes(): metra.aggregateTrainInfo trouble, aborting.");
        return false;
    }
    me.showTrainInfo();
...
/**
 * Returns a UI friendly date string
 * @param  {date|string|number} d  date obj, or msec since epoch, either string or number
 * @param  {boolean} inclSecs      return h:mm:secs if true, else h:mm
 * @return {string}  time          string saying the time, ex: "4:05"
 */
MyCommuteTrain.prototype.uiTime = function(d, inclSecs) {
    var me = this;
    var str;
...
/**
 * returns minutes from now given a time
 *
 * @param {Number} ms - time expressed as msec since epoch.
 * @param {Boolean} [shouldReturnStr=false] - if false, returns number. If true, returns string: "3 mins", "1 min", "< 1 min"
 * @returns {null|Number|String} mins from now, or null if invalid date
 */
MyCommuteTrain.prototype.fromNow = function(ms, shouldReturnStr) {
    var me = this;

    if (typeof ms != 'number') {
        return shouldReturnStr ? '0' : 0;
    }
...
// This has all scheduled and estimated times for a train number:
// http://metrarail.com/metra/wap/en/home/schedules.html?trip-id=MD-N_MN2146_V1_B&line=MD-N&schedule-date=2015-04-30
// http://metrarail.com/metra/wap/en/home/schedules.html?v=more&line=MD-N&start=LAKECOOKRD&end=HEALY&time=now
